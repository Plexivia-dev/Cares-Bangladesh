...
const currentSlug = 'best-preschool-occupational-therapy-dhaka-bangladesh';

export async function generateMetadata() {
  const seo = seoMetadata[currentSlug] || {};
  return {
    title: seo.title || 'Inclusive Preschool Program | Cares Bangladesh',
    description: seo.description || 'Nurturing preschool environment integrating specialized care with standard developmental milestones.',
    alternates: {
      canonical: seo.canonical || 'https://caresbangladesh.com/' + currentSlug,
    },
    openGraph: {
      title: seo.openGraphTitle || seo.title || 'Inclusive Preschool Program',
      description: seo.openGraphDescription || seo.description,
      url: 'https://caresbangladesh.com/' + currentSlug,
      images: [{ url: seo.openGraphImage || '/assets/img/values.jpg' }],
    },
  };
}
...
const ServicePage = () => {
  const prog = programs.find((p) => p.slug === currentSlug) || {"id":"preschool","title":"Inclusive Preschool Program","slug":"best-preschool-occupational-therapy-dhaka-bangladesh","shortDescription":"Nurturing preschool environment integrating specialized care with standard developmental milestones.","badge":"Preschool & Socialization","icon":"GraduationCap","bannerImage":"/uploads/2025/01/Service-Banner_Pre-School-01.png","features":["Structured daily routine with fun interactive activities","Inclusive classroom environment fostering empathy and friendship","School-readiness preparation for smooth school transitions"],"faqs":[{"question":"How does Cares Preschool differ from regular preschools?","answer":"We provide specialized attention, lower student-to-teacher ratios, and integrated therapy support in an inclusive, warm atmosphere."}]};
