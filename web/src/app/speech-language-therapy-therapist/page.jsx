      ...
      <section className="py-16 md:py-20 bg-slate-50/70">
        <div className="container mx-auto px-4 sm:px-6">
          ...
          <div className="max-w-5xl mx-auto mb-12">
            <div className="bg-white p-2 sm:p-2.5 rounded-2xl sm:rounded-full shadow-sm border border-slate-200/90 flex flex-wrap md:flex-nowrap items-center justify-between gap-1.5 sm:gap-2">
              {categories.map((cat) => {
                const Icon = cat.icon;
                const isActive = activeCategory === cat.name;

                return (
                  <div key={cat.name} className="relative flex-1 min-w-[70px] sm:min-w-0">
                    <button
                      onClick={() => setActiveCategory(cat.name)}
                      onMouseEnter={() => setHoveredTooltip(cat.name)}
                      onMouseLeave={() => setHoveredTooltip(null)}
                      className={`w-full py-2.5 px-3 sm:px-4 rounded-xl sm:rounded-full font-sans font-bold transition-all duration-200 cursor-pointer flex items-center justify-center space-x-2 text-xs sm:text-sm ${
                        isActive
                    ...
                    >
                      ...
                      <span className="hidden md:inline lg:hidden whitespace-nowrap">{cat.short}</span>
                    </button>

                    <div className="md:hidden flex justify-center">
                      {hoveredTooltip === cat.name && (
                        <div className="absolute -top-10 left-1/2 -translate-x-1/2 px-2.5 py-1 bg-slate-900 text-white text-[11px] font-medium rounded-lg whitespace-nowrap z-30 shadow-lg pointer-events-none">
                          {cat.name}
            ...
            <div className="text-center mt-3 text-xs font-semibold text-slate-500 font-sans">
