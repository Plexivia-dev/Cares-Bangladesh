...
export default function PageHeader({ title, subtitle, breadcrumb = [] }) {
  return (
    <div className="relative py-20 md:py-24 px-4 overflow-hidden min-h-[260px] flex items-center justify-center bg-[#dbeff8]">
      {/* Authentic Cartoon Background Image (Natural vibrant colors, no dark overlay) */}
      <Image
      ...
      <div className="container mx-auto relative z-10 text-center max-w-4xl space-y-3">
        ...
        {subtitle && (
          <p className="font-sister text-base sm:text-lg md:text-xl text-[#00668f] font-medium max-w-2xl mx-auto">
            {subtitle}
          </p>
        )}

        {/* Breadcrumbs with clean contrast */}
        <div className="inline-flex items-center justify-center space-x-2 text-xs md:text-sm font-semibold pt-1 font-sans text-slate-700 bg-white/70 backdrop-blur-xs px-5 py-1.5 rounded-full shadow-2xs">
