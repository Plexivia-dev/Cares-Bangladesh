...
export const useAuthStore = create((set, get) => ({
  ...
  login: async (email, password) => {
    try {
      ...
      throw new Error(err.message || getApiErrorMessage(err, 'Sign in failed. Please check your credentials.'));
    } finally {
      set({ isLoading: false });
    }
  },
  ...
  verify2fa: async (params, password, code, method = 'authenticator') => {
    try {
      ...
      set({ isLoading: false });
      throw new Error(err.message || getApiErrorMessage(err, '2FA verification failed.'));
    }
  },
  ...
  resendEmailOtp: async (twoFactorToken) => {
    try {
      ...
      throw new Error(err.message || getApiErrorMessage(err, 'Failed to resend verification code.'));
    }
  },
  ...
  setupAuthenticator: async (twoFactorToken) => {
    try {
      ...
      throw new Error(err.message || getApiErrorMessage(err, 'Failed to load Authenticator QR setup.'));
    }
  },
  ...
  sendQrCodeEmail: async (twoFactorToken) => {
    try {
      ...
      throw new Error(err.message || getApiErrorMessage(err, 'Failed to send QR code email.'));
    }
  },
