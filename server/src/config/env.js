...
const envSchema = z.object({
  ...
  ALLOWED_ORIGINS: z.string().default("http://localhost:3000,http://localhost:3001,http://localhost:5173,http://localhost:5174,http://localhost:8001,http://localhost:8005,http://localhost:8090"),
  FRONTEND_DOMAIN_KEYWORDS: z.string().default("localhost,caresbd.com,caresbangladesh.com"),
  DASHBOARD_DOMAIN_KEYWORDS: z.string().default("dashboard,admin,localhost:8005,localhost:5173,localhost:5174"),
  SMTP_HOST: z.string().default("smtp.hostinger.com"),
  SMTP_PORT: z.coerce.number().int().positive().default(587),
  SMTP_ENCRYPTION: z.string().default("TLS"),
  SMTP_USER: z.string().min(1, "SMTP_USER is required"),
  SMTP_PASSWORD: z.string().min(1, "SMTP_PASSWORD is required"),
  SMTP_FROM_NAME: z.string().default("Cares Bangladesh"),
